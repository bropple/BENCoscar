...
type Config struct {
	...
	TOCListeners            []string `envconfig:"TOC_LISTENERS" required:"true" basic:"0.0.0.0:9898" ssl:"0.0.0.0:9898" description:"Network listeners for TOC protocol service. Ignored unless TOC_ENABLED=true.\n\nFormat: Comma-separated list of hostname:port pairs.\n\nExamples:\n\t// All interfaces\n\t0.0.0.0:9898\n\t// Multiple listeners\n\t0.0.0.0:9898,192.168.1.10:9899"`
	TOCEnabled              bool     `envconfig:"TOC_ENABLED" required:"false" basic:"false" ssl:"false" description:"Start the TOC protocol service. OFF unless explicitly enabled, and think before enabling on this deployment: TOC is a plaintext port that carries a reversibly-'roasted' password, and its requests drive the foodgroup services directly without passing through the OSCAR dispatcher -- which means they bypass device attestation entirely. Enabling it reopens both holes on whatever network can reach the listener."`
...
type ICQLegacyConfig struct {
	Enabled            bool          `envconfig:"ICQ_LEGACY_ENABLED" required:"false" basic:"false" ssl:"false" description:"Enable legacy ICQ protocol support (v2-v5). Allows vintage ICQ clients to connect. OFF unless explicitly enabled, and think before enabling on this deployment: the legacy UDP port is plaintext and drives the foodgroup services directly without passing through the OSCAR dispatcher, so it bypasses device attestation entirely. Enabling it reopens both holes on whatever network can reach the listener."`
...
func DefaultICQLegacyConfig() ICQLegacyConfig {
	return ICQLegacyConfig{
		// BENCO: upstream defaults this on. See the Enabled field's description
		// for why this deployment turns it off -- the short version is that the
		// legacy UDP path is plaintext and never passes through the dispatcher
		// where device attestation lives.
		Enabled:            false,
